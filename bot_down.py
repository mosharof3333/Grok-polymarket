...
CHAIN_ID = 137
SIZE = 10.0   # Change this number if you want to trade more/less shares
...
OUTCOME = "Down"   # ← This line has been changed for you
...
def get_current_btc_5m_event():
    ...
    for event in events:
        ...
        if "btc-updown-5m" in slug or ("Bitcoin Up or Down" in title and ("5 min" in title.lower() or "5 minute" in title.lower())):
            return event
    raise ValueError("No active BTC 5-min market found")
    ...
    if "markets" in event and event["markets"]:
        ...
        if "clobTokenIds" in market and "outcomes" in market:
            try:
                ...
                pass
    raise ValueError(f"Could not find {outcome} token ID")
...
def execute_trade(token_id: str, trade_size: float = SIZE):
    ...
    print(f"[{OUTCOME}] Market buy response: {buy_resp}")
...
while True:
    try:
        ...
        if slug != last_slug:                                   # New 5-min window
            ...
            if current_stop_id and not stop_open:               # Stop-loss hit
            ...
            elif current_tp_id and not tp_open:                 # Take-profit hit
